...
import kr.pe.deverexpert.server.listener.socket.worker.ServerSocketWorkerThread;
...
public class ServerSocketQueue {
	...
	static Logger logger = LoggerFactory.getLogger(ServerSocketQueue.class);

    private LinkedList<Object> 	queue = new LinkedList<Object>(); 		            // Request Queue
    private List<ServerSocketWorkerThread> 		threadPool = new ArrayList<ServerSocketWorkerThread>(); 	// Thread Pool Saving Area
    ...
    private ServerSocketHandler 	serverSocketHandler; 		    // Server Socket Handler Class
    
    private boolean 	running = true; 				// Queue Running Flag
    
    
    public ServerSocketQueue( ServerSocketHandler serverSocketHandler, int minThreads, int maxThreads) {
		...
		for( int i = 0; i < this.minThreads; i++ ) {
			...
			ServerSocketWorkerThread thread = new ServerSocketWorkerThread( this, i, serverSocketHandler );
			thread.start();
    ...
    {
        return this.serverSocketHandler.getClass().getName();
    }
    ...
    public synchronized void add( Object o ) throws RequestQueueException {
        ...
        for( Iterator<ServerSocketWorkerThread> i=this.threadPool.iterator(); i.hasNext(); )
        {
            ServerSocketWorkerThread rt = ( ServerSocketWorkerThread )i.next();
            if( !rt.isProcessing() )
            {
        ...
        if( !availableThread ) {
            if( this.currentThreads < this.maxThreads ) {
                ...
                ServerSocketWorkerThread thread = new ServerSocketWorkerThread( this, currentThreads++, this.serverSocketHandler );
                thread.start();
                this.threadPool.add( thread );
            }
            else
